...
const form = ref({ lab_contact_id: '', work_type: 'crown' as WorkType, tooth_reference: '', impression_type: '' as ImpressionType | '', antagonist_info: '', shade: '', sent_date: new Date().toISOString().slice(0, 10), expected_date: '', notes: '' })
...
async function submit() {
  ...
  try {
    await labOrdersApi.create({
      ...
      shade: form.value.shade ? form.value.shade as Shade : null,
...
<template>
  <div class="p-4 space-y-4 max-w-xl">
    <h1 class="text-h2 text-default">{{ t('labOrders.add') }}</h1>
    <p v-if="labs.length === 0" class="text-caption text-subtle">{{ t('labOrders.noLabsHint') }}</p>
    <PatientSearch v-model="selectedPatient" :placeholder="t('labOrders.selectPatient')" />
    <USelect v-model="form.lab_contact_id" :items="labs.map(lab => ({ value: lab.id, label: lab.name }))" :placeholder="t('labOrders.selectLab')" />
    <USelect v-model="form.work_type" :items="workTypes.map(value => ({ value, label: t(`labOrders.workTypes.${value}`) }))" />
    <UInput v-model="form.tooth_reference" :placeholder="t('labOrders.tooth')" />
    <USelect v-model="form.impression_type" :items="impressionTypes.map(value => ({ value, label: t(`labOrders.impressionTypes.${value}`) }))" :placeholder="t('labOrders.impressionType')" />
    <UInput v-model="form.antagonist_info" :placeholder="t('labOrders.antagonistInfo')" />
    <USelect v-model="form.shade" :items="VITA_CLASSICAL_SHADES.map(value => ({ value, label: value }))" :placeholder="t('labOrders.shade')" />
    <UInput v-model="form.sent_date" type="date" />
    <UInput v-model="form.expected_date" type="date" :placeholder="t('labOrders.expectedDate')" />
    <UTextarea v-model="form.notes" :placeholder="t('labOrders.notes')" />
    <div class="flex justify-end gap-2">
      <UButton variant="ghost" to="/lab-orders">{{ t('labOrders.cancel') }}</UButton>
      <UButton :loading="saving" :disabled="!selectedPatient || !form.lab_contact_id" @click="submit">{{ t('labOrders.save') }}</UButton>
    </div>
  </div>
</template>
