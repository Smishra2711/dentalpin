...
(APCI/ALD); that was dropped in `padm_0002` — exemption is now a
...
`patient_admin.relationships.read`/`.write`.
...
`padm_0001`'s migration FK to `patients.id`. `padm_0001` declares
