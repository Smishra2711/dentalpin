...
watch(
  [() => props.document, () => props.initialPatientId],
  ([doc]) => {
    if (doc) {
      ...
      form.patient_id = props.initialPatientId ?? ''
...
onMounted(async () => {
  const response = await api.get<PaginatedResponse<Patient>>('/api/v1/patients', { query: { page_size: 100 } })
  patientOptions.value = response.data.map((p: Patient) => ({
